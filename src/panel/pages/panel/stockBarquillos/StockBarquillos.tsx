...
export const StockBarquillos = () => {
  ...
  const [buscar, setBuscar] = useState<BuscarStockBarquillo>({
      codigo: null,
      movimiento:null,
      fechaCreacion: null,
      buscar:false
  ...
  useEffect(() => {
    ...
    loadStockBarquillo(pageActive!=null ? pageActive : "1", buscar);
      
  }, [pageActive, buscar]);

  const detalle:listaDetalle[] = StockBarquillo.map(({ id, codigo_movimiento, movimiento,  created_at}) => ({
      id: id.toString(),
      campos: [
  ...
  const eliminar = (id:number) => {
    warningDelete(async function(){
      ...
      const result = await deleteStockBarquillo(id);                
  ...
  const { register, handleSubmit, reset, formState:{ errors } } = useForm<FormBuscarStockBarquilloValues>();

  const onSubmit: SubmitHandler<FormBuscarStockBarquilloValues> = ({ codigo, movimiento, fechaCreacion }) => {
